...
const CFG = {
  ...
  maxProps:   Number(process.env.MAX_PROPERTIES          || 5414),
...
function buildFilename() {
  ...
  const citySlug = CFG.city
    .trim()
    .replace(/[^a-zA-Z0-9]+/g, "_")  // spaces, commas, etc → underscore
    .replace(/^_+|_+$/g, "");         // trim leading/trailing underscores
  const dir      = `_data/${citySlug}`;
  mkdirSync(dir, { recursive: true }); // creates _data/ and _data/Phoenix_Arizona/ if missing
  return `${dir}/${yyyy}-${dd}-${mm}-${hh}-${min}-${ss}.json`;
}
...
function exportJSON(results, stats, elapsed) {
  ...
  const payload  = {
    ...
    stats: stats
      ? {
          count:        stats.count,
          averagePrice: parseFloat(stats.avg.toFixed(2)),
          medianPrice:  parseFloat(stats.med.toFixed(2)),
          minPrice:     parseFloat(stats.min.toFixed(2)),
          maxPrice:     parseFloat(stats.max.toFixed(2)),
          belowAverage: stats.below,
          aboveAverage: stats.above,
        }
      : null,
...
const UI = (() => {
  ...
  function footer(elapsed, jsonFile) {
    console.log();
    divider();
    console.log(`  ${chalk.gray("Completed in")} ${chalk.cyan(elapsed)}`);
    console.log(`  ${chalk.gray("Saved to    ")} ${chalk.bold.green(jsonFile)}`);
...
async function main() {
  ...
  // ── JSON export ───────────────────────────────────────────────────────────────
  const jsonFile = exportJSON(results, stats, elapsed);

  UI.footer(elapsed, jsonFile);
}
